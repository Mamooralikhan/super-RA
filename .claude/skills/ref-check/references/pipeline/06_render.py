...
def byline(n, refs_json, author, model):
    """The report is signed. Authorship is not implicit, and the use of AI is not hidden.

    NEVER SHIP THIS WITHOUT THE AUTHOR NAME. If you do not have it, ASK. Quietly dropping a
    byline element because its value was not to hand is a failure, not a graceful degradation.
    That exact omission is why this function takes `author` as a required argument instead of
    defaulting to something bland.
    """
    if not author or not str(author).strip():
        raise SystemExit(
            "FATAL: no author name supplied for the report byline.\n"
            "Ask the user for it. Do not ship an unsigned report, and do not invent a name."
        )
    ...
    return (
        ...
        f'Prepared by <b>{e(author)}</b>, who directs the work and makes every decision. '
        f'Executed by <b>Claude Code</b> ({e(model)}), which fetches, checks, and reports. '
        'Pipeline: <a href="https://github.com/Mamooralikhan/super-RA">super-RA</a> '
        '<code>ref-check</code>. '
...
# --------------------------------------------------------------------------------------------
def render_audit(recs, refs_json, author, model):
    rows = []
...
<tr class="{cls}">
  <td>
    ...
    <p class="note">{cite_from_bib(r['bib_fields'])}</p>
...
{byline(len(recs), refs_json, author, model)}
{GUIDE}
{tiles(recs)}
...
# --------------------------------------------------------------------------------------------
def render_comparison(recs, refs_json, author, model):
    rows = []
    order = {"critical": 0, "major": 1, "decision": 2, "minor": 3, "clean": 4}
    for r in sorted(recs, key=lambda x: (order[x["severity"]], x["key"])):
        sev = r["severity"]
        original = cite_from_bib(r["bib_fields"])
        corrected = cite_from_verified(r.get("verified_metadata"))
...
def main():
    ...
    ap.add_argument("--author", required=True,
                    help="Name for the report byline. ASK THE USER. Never guess it.")
    ...
    OUT_AUDIT.write_text(render_audit(recs, refs_json, args.author, args.model), encoding="utf-8")
    OUT_COMPARE.write_text(
        render_comparison(recs, refs_json, args.author, args.model), encoding="utf-8")
