...
# Appendix: maintaining this repository

The block above is the super-RA brain. It governs this repository too, because super-RA holds itself to the contract it ships. It is embedded here **verbatim** from `brain/CLAUDE.md`; the validator hashes both and fails if they diverge. Edit `brain/CLAUDE.md` first, then copy the block between the markers into this file.

## What this repository is

A public library of research-assistant skills for empirical social science, plus the brain that governs a project once super-RA is installed into it.

- `brain/` is the constitution, the enforcement settings, and the session notice. It is what `scripts/install_super_ra.sh` copies into a research project.
- `context/OPERATING-CONTRACT.md` is the canonical Operating Contract, embedded verbatim in every skill, and it is what carries super-RA's rules into a skill invoked in a folder that super-RA does not govern.
- `.claude/skills/` holds the three skills.

## The invariants, and why they are not negotiable

Run the validator after every edit. It is not advisory.

```sh
sh scripts/validate_skills.sh
```

- **No em-dashes**, anywhere in the README, the context, the brain, or the skills. The build fails on a single literal one. The `&mdash;` HTML entity is ASCII and passes.
- **The Operating Contract is identical** in `context/OPERATING-CONTRACT.md` and in all three `SKILL.md` files. Edit the canonical copy first, then copy it out.
- **The brain block is identical** in `brain/CLAUDE.md` and in this file.
- **Every shipped pipeline template compiles.**
- **Codex stays removed.** It was removed because `ref-check` depends on subagents, and one agent reviewing its own work is not an independent check. The word survives in the README and the WORKLOG on purpose, so that someone who goes looking for Codex finds an answer rather than a mysterious absence. The validator greps for *functional* references, not for the word.

## Traps

**Do not use `git checkout -- <file>` to undo an experiment.** `HEAD` may hold an older state of a skill, and the checkout will silently revert current work rather than undoing your edit. Take a backup copy instead.

**The validator can match its own source.** It names the forbidden strings in order to forbid them, so it excludes itself with `--exclude`. If you add another forbidden-string check, exclude the validator again, or it will fail permanently against itself and look like a real finding.

**`01_extract_citations.py` is the most dangerous file here.** Every guard in it was learned from a bug that produced a confident, well-formatted, *false accusation against a bibliography that was correct*. A bad extractor does not fail loudly; it invents a defect and the tiers below it earnestly investigate a problem that never existed. Read `references/extraction-rules.md` before touching it, and re-run the known-good numbers afterwards.
